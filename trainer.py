...
class Trainer():
    ...
    def train(self, epoch_num, lr=0.0001, load_from_disk=False, epochs_per_val=5, save_val=False):
        gc.collect()
    ...
    def train_step(self, train_loader, save=False, transform_out=None) -> float:
        ...
        for name, bunch_lr, bunch_hr in (pbar := tqdm(train_loader)):
            ...
            bunch_lr, bunch_hr = bunch_lr.to(self.device), bunch_hr.to(self.device)
            
            output_hr, weights_hr = self.model(bunch_lr, bunch_hr)
