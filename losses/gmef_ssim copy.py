...
def create_grad_window(window_size, channel):
    ...
    kernel_x = kernel_x.expand(channel, channel, window_size, window_size).float().contiguous()
    kernel_y = kernel_y.expand(channel, channel, window_size, window_size).float().contiguous()
...
def _gmef_ssim(output, bunch, ws):
    ...
    mefssim, patch_index = mef_ssim(output, bunch, ws, is_lum=True)

    ref_idx = len(bunch) // 2
    ref = bunch[ref_idx].unsqueeze(0)
    grad_ssim_abs, grad_ssim_ang = calc_grad_ssim(output, ref, ws)
    
    grey_ref = ref
    mask = (grey_ref > 1/32) & (grey_ref < 31/32)
    grad_ssim_abs_m = grad_ssim_abs[mask].mean()
    grad_ssim_ang_m = grad_ssim_ang[mask].mean()

    # grad_ssim_abs[~mask] = 1.0
    # grad_ssim_ang[~mask] = 1.0
    # plot_weights(bunch.permute(0, 2, 3, 1), grad_ssim_abs.view(H, W), grad_ssim_ang.view(H, W))

    return mefssim * grad_ssim_abs_m * grad_ssim_ang_m
