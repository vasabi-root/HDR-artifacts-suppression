...
def _mef_ssim(output, bunch, window, ws, denom_g, denom_l, C1, C2, is_lum=False, full=False):
...
def mef_ssim(output, bunch, window_size=11, is_lum=False):
    ...
    return _mef_ssim(output, bunch, window, window_size, 0.08, 0.08, 0.01**2, 0.03**2, is_lum)


def mef_msssim(output, bunch, window, ws, denom_g, denom_l, C1, C2, is_lum=False):
    ...
    for _ in range(levels):
        l, cs = _mef_ssim(output, bunch, window, ws, denom_g, denom_l, C1, C2, is_lum=is_lum, full=True)
        l_i.append(l)
        cs_i.append(cs)
