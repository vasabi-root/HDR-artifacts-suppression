...
def calc_grad_ang_ssim(output, bunch, window_size):
    K, C, H, W = bunch.shape
    _ , grad_Y_ang  = calc_grad(bunch, window_size)
    _ , grad_X_ang  = calc_grad(output, window_size)
    _ , grad_X_sq_ang  = calc_grad(output**2, window_size)
    _ , grad_Y_sq_ang  = calc_grad(bunch**2, window_size)
    _ , grad_XY_ang  = calc_grad(output * bunch, window_size)
    ...
    return grad_ssim_ang


def gssim(output, bunch, ws, return_map=False):
    ...
    grad_ssim_ang = calc_grad_ang_ssim(output, bunch, ws)
    ...
    grad_ssim_ang_full = grad_ssim_ang[ref_idx].clone(); 
    if len(~mask) > 0 and (~mask).all() != False:
        grad_ssim_ang_full[~mask] = grad_ssim_ang_patch[~mask]
    ...
    # plot_bunches(maps, ghosts, save_name=Path('plots/gssim_.png'))


    metric = grad_ssim_ang_full.mean()
...
class GSSIM(torch.nn.Module):
    def __init__(self, window_size=11):
        super().__init__()
        self.window_size = window_size

    def forward(self, output, bunch):
        return gssim(output, bunch, self.window_size)
...
def test_meflut():
    ...
    l_maps = []
    for ws in [3, 7, 11, 15]:
        l_map, cs_map, _ = gssim(output, bunch, ws, return_map=True)
