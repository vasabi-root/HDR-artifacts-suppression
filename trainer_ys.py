...
class Trainer():
    ...
    def train_step(self, train_loader, save=False, transform_out=None) -> float:
        ...
        for name, bunch_lr, bunch_hr in (pbar := tqdm(train_loader)):
            batch_size = len(bunch_lr)
            bunch_lr, bunch_hr = bunch_lr.to(self.device).squeeze(0), bunch_hr.to(self.device).squeeze(0)

            Y_lr, _,_ = bunch_to_Y_CbfCrf(bunch_lr)
            Y_hr, Cb_f, Cr_f = bunch_to_Y_CbfCrf(bunch_hr)
            
            Y_f, weights_hr = self.model(Y_lr, Y_hr)
            output_hr = torch.cat((Y_f, Cb_f, Cr_f), dim=1)

            loss = -self.metric(output_hr, torch.squeeze(bunch_hr, dim=0))
            run_loss += loss.item()
            ...
            if save:
                self.save_results(output_hr, weights_hr, name, is_train=True)

            pbar.set_postfix({'gmef_ssim ': f' {-loss:.3}',})

            torch.cuda.empty_cache()
            # break

        train_loss = run_loss / len(train_loader)
        print(f'  train gmef_ssim:\t {-train_loss:.3}')
    ...
    def valid_step(self, valid_loader, save=False, transform_out=None) -> float:
        ...
        with torch.no_grad():
            for name, bunch_lr, bunch_hr in (pbar := tqdm(valid_loader)):
                batch_size = len(bunch_lr)
                bunch_lr, bunch_hr = bunch_lr.to(self.device).squeeze(0), bunch_hr.to(self.device).squeeze(0)
                ...
                loss = -self.metric(Y_f, torch.squeeze(Y_hr, dim=0))
                run_loss += loss.item()

                if save:
                    self.save_results(output_hr, weights_hr, name, is_train=False)
        ...
        print(f'  valid gmef_ssim:\t {-valid_loss:.3}')
