...
class EfficientHDR(nn.Module):
    def __init__(self, in_channels=3, num_exposures=3, embed_dim=64):
        ...
        self.feature_fusion = nn.Conv2d(embed_dim * num_exposures, embed_dim, 3, 1, 1)
        
        # Блоки внимания на основе EfficientViT
        self.fused_attention = EfficientViTBlock(embed_dim) 
    ...
    def forward(self, exposures):
        ...
        for i in range(num_exposures):
            if i != ref_idx:
                ...
                # spatial attention
                attended = self.pairs_attention[i if i < ref_idx else i-1](other_features, ref_features)
                attended_features.append(attended)
            else:
                attended_features.append(ref_features)
        
        # Объединение обработанных признаков
        concated_features = torch.cat(attended_features, dim=1)  # [B, feature_dim * (num_exposures - 1), H, W]
        fused_features = self.feature_fusion(concated_features)
        attended = self.fused_attention(fused_features)
        
        # Генерация HDR-изображения
        output = self.tail(attended+fused_features)
...
if __name__ == "__main__":
    ...
    model = EfficientHDR(in_channels=3, num_exposures=3, embed_dim=32).to(device) # 3899
    sample_input = torch.randn(1, 3, 3, 1280, 720).to(device)
    output = model(sample_input)
    print(output.shape)
    summary(model, sample_input.shape[1:])
    # model = EfficientViTBlock(32).to(device)
    # summary(model, (32, 640, 480), batch_size=1)