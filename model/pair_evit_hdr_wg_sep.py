...
class WeightsExtractor(nn.Module):
    def __init__(self, in_channels=3, num_exposures=3, embed_dim=64):
        ...
        self.light_weight = nn.Sequential(
            self.norm,
            nn.Conv2d(embed_dim, 1, 3, 1, 1),
            nn.Sigmoid(),
        )
        self.light_embed =  nn.Sequential(
            nn.Conv2d(1, embed_dim, 3, 1, 1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, exposures):
        ...
        for i in range(num_exposures):
            ...
            light_weight = self.light_weight(features)
            light_weights.append(light_weight)
        
        # Извлечение признаков опорного кадра
        ref_features = self.light_embed(light_weights[ref_idx]) # [B, feature_dim, H, W]
        
        # Обработка неопорных кадров через блоки внимания
        attended_features = []
        for i in range(num_exposures):
            if i != ref_idx:
                other_features = self.light_embed(light_weights[i])
        ...
        weights = []
        for light, motion, tail in zip(light_weights, attended_features, self.tails):
            weights.append(tail(motion))

        return torch.stack(light_weights, dim=1), torch.stack(weights, dim=1)
...
class PairEVitHDR(nn.Module):
    ...
    def forward(self, exposures_lr, exposures_hr):
        light_weights_lr, weights_lr = self.weights_extractor(exposures_lr)
        ...
        light_weights_hr = self.gf(lights_lr, light_weights_lr[0], lights_hr).unsqueeze(0)

        output_hr = (weights_hr*exposures_hr).sum(1) / clamp_min(weights_hr.sum(1))
        light_hr = (light_weights_hr*exposures_hr).sum(1) / clamp_min(light_weights_hr.sum(1))
        return output_hr, light_hr, weights_hr
...
if __name__ == "__main__":
    ...
    output, _, _ = model(sample_input_lr, sample_input_hr)
    # print(output.shape)
